...
void fpscounter_reset(FPSCounter *fps) {
	hrtime_t frametime = TIME_RESOLUTION / FPS;
	...
	fps->fps = TIME_RESOLUTION / (long double)frametime;
...
void fpscounter_update(FPSCounter *fps) {
	...
	for(int i = 0; i < log_size; ++i) {
		avg += fps->frametimes[i] * ((pow(i + 1.0, 3) / pow(log_size + 1.0, 2)) / (0.25 * log_size));
	}

	fps->fps = TIME_RESOLUTION / (avg / log_size);
	...
	fps->fps = TIME_RESOLUTION / (avg / (long double)log_size);
	fps->frametime = avg / log_size;
#endif
...
void loop_at_fps(LogicFrameFunc logic_frame, RenderFrameFunc render_frame, void *arg, uint32_t fps) {
	...
	hrtime_t target_frame_time = TIME_RESOLUTION / fps;
...
begin_frame:
		...
		if(sleep > 0) {
			...
			while((shrtime_t)next_frame_time - (shrtime_t)time_get() > (shrtime_t)target_frame_time / sleep) {
				...
				uint32_t nap_sdl = (nap_multiplier * nap_raw * 1000) / (TIME_RESOLUTION * nap_divisor);
